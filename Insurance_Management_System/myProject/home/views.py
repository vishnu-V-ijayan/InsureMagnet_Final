...
from django.shortcuts import render, redirect
from django.utils.crypto import get_random_string
from .models import Staff, User  # Ensure User is properly imported
from django.views.decorators.cache import never_cache
...
def register_staff(request):
    if request.method == "POST":
        ...
        user = User.objects.create_user(
            username=email, 
            email=email,
            password=password,  # Automatically handles hashing
            role=User.Role.STAFF
        )
        user.save()
        ...
        return HttpResponse("Staff member added successfully")  # Redirect or inform of success
    else:
...
from django.contrib import messages
from django.contrib.auth.decorators import login_required
from .models import Customer, VehiclePlan, Office  # Updated to include VehiclePlan
...
def agent_select_customer_vehicle(request):
    ...
    if customer_id:
        try:
            customer = Customer.objects.get(id=customer_id)
            plans = VehiclePlan.objects.all()  # Fetching data from VehiclePlan model
            offices = Office.objects.all()  # Fetching data from Office model
            years = range(1980, 2025)  # Example range for vehicle manufacturing year
            ...
            return render(request, 'agent/agent_vehicle_policyissue.html', context)
        except Customer.DoesNotExist:
            messages.error(request, "Customer does not exist.")
            return redirect('agent_list_customers')  # Redirect back to customer list
    else:
        messages.error(request, "Invalid customer selection.")
        return redirect('agent_list_customers')  # Redirect back to customer list
...
def agent_register_vehicle_policy(request):
    ...
    customer_id = request.session.get('customer_id')  # Fetch customer ID from session
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            policy = VehiclePolicy(
                customer=customer,
                agent=agent,  # Storing the logged-in user as agent
                vehicle_number=vehno,
                vehicle_value=vehvalue,
                vehicle_plan=plan,
                engine_number=enginno,
            ...
            policy.save()
            logger.info(f"Vehicle Policy for {vehno} registered by agent {agent.name}.")
            return redirect('agent_policy_success')  # Adjusted redirect
        except (VehiclePlan.DoesNotExist, Office.DoesNotExist, ValueError) as e:
            logger.error(f"Error processing your request: {e}")
            return HttpResponse(f"Error processing your request: {e}", status=400)
    else:
...
def agent_calculate_policy_amount(vehvalue, plan):
    depreciation_factor = Decimal(plan.depreciation / 100)
    plan_percentage = Decimal(plan.Ppercentage / 100)  # Changed from 'percentage' to 'Ppercentage'
    return (vehvalue - (vehvalue * depreciation_factor)) * (1 + plan_percentage)
...
@login_required
def agent_policy_success(request):
    try:
        customer_id = request.session.get('customer_id')
        customer = Customer.objects.get(id=customer_id)
        vehicle_policy = VehiclePolicy.objects.filter(customer=customer).order_by('-policy_date', '-id').first()
    ...
    return render(request, 'agent/agent_vehicle_policyissue_successful.html', context)


@login_required
def agent_vehicle_premium_payment(request):
    # Placeholder for actual payment processing page
    return render(request, 'agent/agent_vehicle_premium_payment.html')

@csrf_exempt
def agent_paymenthandler(request):
    if request.method != "POST":
        return HttpResponseBadRequest("Invalid request method")
    ...
    try:
        ...
        vehicle_policy_id = request.POST.get('vehicle_policy_id')  # Expect policy ID in POST data
        vehicle_policy = VehiclePolicy.objects.get(id=vehicle_policy_id)
        ...
        logger.error("Vehicle Policy not found with provided ID")
        return HttpResponseBadRequest("Vehicle Policy not found")
    except Exception as e:
        logger.error(f"Error processing the payment: {e}")
        return HttpResponseBadRequest(f"Error in processing your payment: {e}")
...
@login_required
def agent_generate_policy_qr(request, policy_id):
    try:
        vehicle_policy = VehiclePolicy.objects.get(id=policy_id)
        customer = vehicle_policy.customer
    except VehiclePolicy.DoesNotExist:
        return HttpResponse("Vehicle policy not found.", status=404)

    user_name = f"{customer.first_name} {customer.last_name}".strip()
    qr_data = f"Agent Handling: {request.user.username}\nUser: {user_name}\nPolicy Number: {vehicle_policy.id}\nVehicle Number: {vehicle_policy.vehicle_number}\nVehicle Value: {vehicle_policy.vehicle_value}\nPolicy Amount: {vehicle_policy.policy_amount}\nPolicy End Date: {vehicle_policy.policy_end_date}"
...
def agent_generate_policy_pdf(request, policy_id):
    try:
        vehicle_policy = VehiclePolicy.objects.get(id=policy_id)
    except VehiclePolicy.DoesNotExist:
        return HttpResponse("Policy not found")
